...
import { getUserProfile, type UserProfile } from '@/app/lithium/profile';
      ...
      <div className="hidden md:block border-b border-neutral-200/80 dark:border-neutral-800/80 bg-white/80 dark:bg-[#191919]/80 backdrop-blur-md sticky top-0 z-40 transition-colors">
        ...
        <div className="px-6 py-2.5 border-b border-neutral-100 dark:border-neutral-800/60 flex items-center justify-between gap-4 text-xs text-neutral-600 dark:text-neutral-400">
          ...
          <div className="flex items-center gap-2 justify-end flex-1">
            ...
            {/* User Auth Profile Button (Trigger for Settings Dialog, matching Lithium NavUser) */}
            {userProfile ? (
              <button
                type="button"
                onClick={() => setIsSettingsOpen(true)}
                className="flex items-center gap-2 px-2 py-1 h-7 rounded-md hover:bg-neutral-100 dark:hover:bg-neutral-800 text-neutral-800 dark:text-neutral-200 transition-colors cursor-pointer border border-transparent hover:border-neutral-200/70 dark:hover:border-neutral-700/70 max-w-[180px]"
                title="Account Settings"
              >
                <Avatar className="h-5 w-5 rounded-md shrink-0">
                  {userProfile.avatarUrl && <AvatarImage src={userProfile.avatarUrl} alt={userProfile.displayName || "Avatar"} />}
                  <AvatarFallback className="rounded-md text-[10px] font-medium">
                    {(userProfile.displayName || userProfile.email).slice(0, 1).toUpperCase()}
                  </AvatarFallback>
                </Avatar>
                <span className="truncate text-[11px] font-medium">
                  {userProfile.displayName || userProfile.email || "User"}
                </span>
                <ChevronsUpDown className="w-3 h-3 text-neutral-400 shrink-0 ml-auto" />
              </button>
            ) : (
              <button
                type="button"
